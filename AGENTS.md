...
## 1. 定位
Windows 便携工具：框选屏幕区域 → 离线 OCR（RapidOCR+ONNX）监控文字变化 → 写记录 txt；按「屏幕文字→指令」规则经串口/TCP 给 ESP32 发开关指令；可选 DeepSeek 视觉"看图说话"。全程 OCR 离线、数据不出本机。

## 2. 结构
| 文件/目录 | 说明 |
| ---- | ---- |
| main.py | 全部主程序（55KB，含界面/OCR/驱动/视觉/打包入口） |
